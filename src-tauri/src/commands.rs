...
use crate::database::repositories::{dictionary::DictionaryRepository, transcript::TranscriptRepository};
...
impl From<AuthError> for ApiError {
  fn from(value: AuthError) -> Self {
    match value {
      ...
      AuthError::InvalidCredentials => Self::new("invalid_credentials", "invalid credentials"),
...
pub async fn save_transcript(
  ...
  let repo = TranscriptRepository::new(state.pool.clone());
  let transcript = repo
    .create(CreateTranscript { content })
    .await?;
  Ok(transcript.into())
}
...
pub async fn show_main_window(app: AppHandle) -> Result<(), ApiError> {
  if let Some(window) = app.get_webview_window("main") {
    window.show().map_err(|e| ApiError::new("window_error", e.to_string()))?;
    window.set_focus().map_err(|e| ApiError::new("window_error", e.to_string()))?;
  }
  Ok(())
}
...
pub async fn hide_main_window(app: AppHandle) -> Result<(), ApiError> {
  if let Some(window) = app.get_webview_window("main") {
    window.hide().map_err(|e| ApiError::new("window_error", e.to_string()))?;
  }
  Ok(())
}
...
#[tauri::command]
pub async fn register_hotkey(app: AppHandle, state: State<'_, AppState>, hotkey: String) -> Result<bool, ApiError> {
...
pub async fn get_registered_hotkeys(state: State<'_, AppState>) -> Result<Vec<String>, ApiError> {
  ...
  Ok(current.as_ref().map(|h| vec![h.clone()]).unwrap_or_default())
}

...
/// Disables the ladder: a streaming pass is decoded again before anything is
/// committed, so rescuing one buys nothing.
const TEMPERATURE_INC_OFF: f32 = 0.0;
const LOGPROB_THOLD: f32 = -1.0;

/// Streaming passes are provisional; the final one is what the user keeps.
